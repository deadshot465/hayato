...
def search_line(author_name: str,
    ...
    if len(found_lines) > 1:
        return __build_multiple_result_embed(author_name, author_avatar_url, color, found_lines)
    elif len(found_lines) == 0:
        return __fuzzy_search(author_name, author_avatar_url, color, keyword, lines, railway_name)
    else:
        line = found_lines.pop()
        return build_single_result_embed(author_name, author_avatar_url, line,
...
@lightbulb.command('rails', 'Ask Hayato for information on trains and rails.')
@lightbulb.implements(lightbulb.SlashCommandGroup)
async def rails(ctx: lightbulb.Context) -> None:
    pass
